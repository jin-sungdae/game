...
Full `validate:alpha:release` is **FAIL / BLOCKED**, not PASS: the frozen runtime inventory detects the requested shared RAF call-site change. Updating `docs/evidence/alpha-release-runtime-inventory.json` was rejected by automatic approval review because it requires explicit user approval. The baseline and policy remain unchanged; no bypass. The pending request is limited to registering `animation/clock.ts` and removing the replaced legacy `useAnimation.ts` RAF sites. Native focus/network/worker protections stay intact. This draft is not READY_FOR_HUMAN_REVIEW until that approval, full gate and latest CI pass. Architecture still requires human review before merge.
