...
const {hpModel,effectFor,damageFor}=require(path.join(process.env.LUMA_PRESENTATION_TEST_DIR,'presentation/model.js'));
test('HP tiers include exact thresholds and retain numbers',()=>{for(const [hp,tone] of [[100,'normal'],[51,'normal'],[50,'warning'],[25,'warning'],[24,'critical'],[0,'critical']]){assert.equal(hpModel(hp,100).tone,tone);assert.equal(hpModel(hp,100).text,`${hp} / 100`);}assert.equal(hpModel(1,0).percent,0);});
