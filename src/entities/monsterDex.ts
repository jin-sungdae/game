...
export const archetypes = ['BEAST', 'SLIME', 'PLANT', 'SPIRIT', 'BIRD', 'INSECT', 'AQUATIC', 'ROCK', 'SHADOW', 'MECHANICAL', 'MIMIC', 'COSMIC', 'FIRE', 'MOON', 'NIGHT'] as const;
...
export function parseMonsterDefinitions(input: unknown): readonly MonsterDefinition[] {
  ...
  return Object.freeze(input.map((value: unknown): MonsterDefinition => {
    ...
    const result: MonsterDefinition = {
      ...
      visualScale: number(v.visualScale, 0.5, 1.5), enabled: boolean(v.enabled), contentReady: boolean(v.contentReady), alphaCandidate: boolean(v.alphaCandidate),
