...
import { hpModel, monsterWaitMessage } from '../presentation/model';
...
export function Interaction() {
 ...
 const result=v?.phase==='CAPTURE_SUCCESS'?'Captured!':v?.phase==='CAPTURE_FAIL'?'Capture failed':b?.status==='VICTORY'?(active?`${monsterName} defeated · Capture opportunity`:'Battle complete'):b?.status==='DEFEAT'?'MOA needs a rest':b?.encounterStatus==='CAPTURED'?'Added to Collection':monsterWaitMessage(b,monsterName)??b?.status??'A small encounter';
