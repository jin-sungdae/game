...
const CODES: [&str; 5] = ["PIP", "MELLO", "MOSSY", "CHIRP", "BUBU"];
fn world() -> World {
    let mut w = World::new(
        Area {
            x: -4000.0,
...
fn motion(w: &mut World, code: &str) {
    ...
    let ground = w.area.ground_y();
    let mut max_y = start.y;
    w.tick(0.6, 0.1, (-9999.0, -9999.0), false);
    for i in 1..=40 {
        ...
        assert!(
            b.y >= ground && b.y + b.h <= w.area.y + w.area.h - 8.0,
            "{code} y"
        );
        assert!([-1, 1].contains(&p.facing));
        if code == "CHIRP" {
            assert!(p.y > ground + 500.0, "flying must stay in top region");
        }
    }
    let p = w.view.pip.as_ref().unwrap();
...
fn all_five_production_placement_movement_identity_and_restart() {
    for code in CODES {
        ...
        let a = w.area;
        if code == "CHIRP" {
            assert_eq!(p.y, (a.y + a.h - 8.0 - p.size.height).floor());
        } else {
...
fn missing_asset_unsupported_content_and_unsafe_placement_never_discover() {
    for code in CODES {
        ...
        e.monster.movement_profile = "STATIC".into();
...
#[test]
#[ignore = "requires isolated test-only LUMA_TEST_BATCH1 Spring fixture"]
fn live_batch1_world_capture_and_collection() {
